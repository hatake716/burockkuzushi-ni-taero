...
- スクリーンショット・映像：本アプリ1.0.4のAPI 35 Androidエミュレーター実画面。入力スクリプトは `StoreCaptureTest.kt`。背景やHUDの合成、ゲーム内部値の演出用変更は行っていない。
...
- ゲーム画面・グラフィック・動画字幕：DotGothic16 Regular。Copyright 2020 The DotGothic16 Project Authors、SIL Open Font License 1.1。[配布元とハッシュ](../FONTS.md)。AAB/APKに元のフォントとライセンスを同梱。ZIPにはライセンスも別途収録します。
